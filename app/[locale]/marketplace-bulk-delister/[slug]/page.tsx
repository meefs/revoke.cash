...
import MarketplaceBulkDelisterChainSelect from './MarketplaceBulkDelisterChainSelect';
import MarketplaceBulkDelisterChecker from './MarketplaceBulkDelisterChecker';
...
export const generateMetadata = async ({ params }: Props): Promise<Metadata> => {
  ...
  return {
    title: t('signatures.marketplace.meta.title', { chainName }),
    description: t('common.meta.description', { chainName }),
    openGraph: {
      images: getOpenGraphImageUrl(`/marketplace-bulk-delister/${slug}`, locale),
    },
  };
};

const MarketplaceBulkDelisterPage: NextPage<Props> = async ({ params }) => {
  const { locale, slug } = await params;
...
export default MarketplaceBulkDelisterPage;
